...
object KuduContext {
   ...
   *  underlining wrapper all foreach functions in HBaseContext
   */
  def kuduForEachPartition[T](context:KuduContext, 
                              it: Iterator[T], 
                              f: (Iterator[T], KuduClient, AsyncKuduClient) => Unit) = {
    f(it, KuduClientCache.getKuduClient(context.broadcastedKuduMaster.value),
    KuduClientCache.getAsyncKuduClient(context.broadcastedKuduMaster.value))
  }
   ...
   *             with HBase
   */
  def foreachPartition[T](context:KuduContext,
                          rdd: RDD[T],
                          f: (Iterator[T], KuduClient, AsyncKuduClient) => Unit):Unit = {
    rdd.foreachPartition(
      it => KuduContext.kuduForEachPartition(context, it, f))
  }
   ...
   *                 definition function just like normal mapPartition
   */
  def streamForeachPartition[T](context:KuduContext,
                                dstream: DStream[T],
                                f: (Iterator[T], KuduClient, AsyncKuduClient) => Unit): Unit = {
